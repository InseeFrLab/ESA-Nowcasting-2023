...
for (country in countries$geo) {
  ...
  pred_next_month <- pred$predictions * std_ppi + mean_ppi
  ...
  pred_residuals <- predictions$predictions * std_ppi + mean_ppi
  
  y_pred_residuals <- df_country$PPI - pred_residuals
