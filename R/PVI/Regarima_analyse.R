...
sarima(dpvi, 0, 0, 1 , xreg=cbind(IPT1,dIPT1,IS1,dIS1,
                                  ind2003,ind2004,ind2005,ind2006), details = FALSE)
...
myspec1 <- regarima_spec_tramoseats(spec = "TR3",
                                    usrdef.varEnabled = TRUE, usrdef.var = var)
myreg1 <- regarima(pvi_ts,myspec1)

myreg1$forecast

myspec2 <- regarima_spec_tramoseats(arima.coefType="Undefined",
                                    arima.p=0,
                                    arima.q=1,
                                    arima.bp=0,
                                    arima.bq=0,
...
brent <- data$brent %>% mutate(time = floor_date(time, unit = "month")) %>% 
  group_by(time) %>% 
  summarize(brent_adjusted=mean(brent_adjusted,na.rm=TRUE)) %>% 
  select(c(time,brent_adjusted))
debut <- brent%>%slice(1:1)%>%pull(time)
debut<-c(year(debut),month(debut))
...
ppi_spec <- regarima_spec_tramoseats(estimate.from="2010-01-01",
                                     automdl.enabled=TRUE,
                                     outlier.enabled=TRUE,
                                     outlier.ao=TRUE,
                                     outlier.tc=FALSE,
