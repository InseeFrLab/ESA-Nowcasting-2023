...
error_country_model <- past_submissions_ppi |>
  tibble::tibble() |>
  dplyr::rename(geo = Country, time = Date, forecast = value) |>
  dplyr::inner_join(recent_data_ppi, by = c("geo", "time")) |>
  dplyr::mutate(error_squared = (forecast - values)^2 / values) |>
  tidyr::drop_na(error_squared) |>
...
ojs_define(data = recent_data_ppi, pred = past_submissions_ppi, errors_data = error_country_model, ave_errors_data = average_error)
```
